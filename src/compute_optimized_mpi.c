    ...
    for (int c = 0; c < num_cols; c ++) {
        ...
        for (int i = r; i < (flip_matrix->rows + r); i++) {
            ...
            if(j > c) {
                ...
                __m256i a_vec;
                __m256i mul_vec;
                for (int j = c; j ((flip_matrix->cols)/32*32)+c; j+= 32) {
                    ...
                    __m256i flip_vec2 = _mm256_loadu_si256( (__m256i *) (flip_matrix->data + 8 + (j-c) + (col_b*(i-r+4))));
                    __m256i a_vec2 = _mm256_loadu_si256( (__m256i *) (a_matrix->data + 8 + j + (col_a*i)));
                    __m256i mul_vec2 = _mm256_mullo_epi32(flip_vec2, a_vec2);
                    sum_vec = _mm256_add_epi32(sum_vec, mul_vec);

                    __m256i flip_vec3 = _mm256_loadu_si256( (__m256i *) (flip_matrix->data + 16 + (j-c) + (col_b*(i-r+8))));
                    __m256i a_vec3 = _mm256_loadu_si256( (__m256i *) (a_matrix->data + 16 + j + (col_a*i)));
                    __m256i mul_vec3 = _mm256_mullo_epi32(flip_vec3, a_vec3);
                    sum_vec = _mm256_add_epi32(sum_vec, mul_vec);

                    __m256i flip_vec4 = _mm256_loadu_si256( (__m256i *) (flip_matrix->data + 24 + j-c) + (col_b*(i-r+12)));
                    __m256i a_vec4 = _mm256_loadu_si256( (__m256i *) (a_matrix->data + 24 + j + (col_a*i)));
                ...
                for (; j < ((flip_matrix->cols)/8*8)+c; j+= 8) {
                  ...
                  sum_vec = _mm256_add_epi32(sum_vec, nul_vec);
                }
